...
void TerrainInternalWrapper(uint64_t worldSeed, int32_t chunkX, int32_t chunkZ, int posX, int posZ) {
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        int index=0;
        do random_next_int(&worldRandom, 256u - index); while (index++ != 255);
    }
    for (int i = 0; i < 16; ++i) {
        advance6(&worldRandom);
        int index=0;
    ...
    //crucial
    initOctaves(shoresBottomComposition, &worldRandom, 4);
    initOctaves(surfaceElevation, &worldRandom, 4);
    ...
    for (const auto &octave : shoresBottomComposition) {
    ...
    octavesFactor = 1.0;
    for (const auto &octave : surfaceElevation) {
...
int main() {
    ...
    TerrainInternalWrapper(18420882071630, -3, 6, 1,6);
}